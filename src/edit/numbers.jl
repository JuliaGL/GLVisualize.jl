...
immutable NumberTexture{T, NDim}
    text::Texture{GLGlyph{Uint16}, NDim}
    arr::Array{T, NDim}
    maxdigits::Int
end

function setindex!(t::NumberTexture, value::Real, i::Integer)
    str = makestring(value, t.maxdigits)
    t.arr[i] = value
    tex_index = i*t.maxdigits
    t.tex[tex_index:tex_index+t.maxdigits] = str
end

function NumberTexture(arr::Array{Real})
    Array(GLGlyph{Uint16}, int(xn*maxdigits), yn)
end


function edit{T <: Union(AbstractVector, Real)}(snumbers::T, style::Style, customization::Dict{Symbol,Any})
