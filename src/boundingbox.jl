...
function call{T, R, N}(B::Type{AABB{T}}, grid::Grid{N, R}, scale::Void, primitive_bb::AABB{T})
    ...
    pmax = max(primitive_scaled_min, primitive_scaled_max)
    pmin = min(primitive_scaled_min, primitive_scaled_max)
    mainmin = Vec(Vec{N, Float32}(map(first, grid.dims)), ntuple(x->0f0, 3-N)...)
    mainmax = Vec(Vec{N, Float32}(map(last, grid.dims)), ntuple(x->0f0, 3-N)...)
    B(mainmin + pmin, mainmax + pmax)
end

function call{T, N1,N2}(B::Type{AABB{T}}, positions::Vector{Point{N1, T}}, scale::Vector{Vec{N2, T}}, primitive_bb::AABB{T})
