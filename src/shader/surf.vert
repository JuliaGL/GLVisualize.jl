...
void main()
{
	vec3 pos 		= position(Rectangle(grid_min, grid_max), textureSize(z, 0), gl_InstanceID);
	float intensity = linear_texture(z, gl_InstanceID).x;
	pos += vertex;
	pos += vec3(0, 0, intensity);

	vec4 instance_color = color(intensity, color_ramp, norm);
	render(pos, linear_texture(normal, gl_InstanceID).xyz, instance_color, viewmodel, projection, light);
}
