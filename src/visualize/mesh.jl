_default{M<:GLNormalAttributeMesh}(mesh::TOrSignal{M}, s::Style, data::Dict) = @gen_defaults! data begin
    main 		= mesh
    boundingbox = const_lift(GLBoundingBox, mesh)
    shader 		= GLVisualizeShader("util.vert", "attribute_mesh.vert", "standard.frag")
end

_default{M<:GLNormalMesh}(mesh::TOrSignal{M}, s::Style, data::Dict) = @gen_defaults! data begin
    main 		= mesh
...
_default{M<:GLPlainMesh}(main::TOrSignal{M}, ::style"grid", data::Dict) = @gen_defaults! data begin
    primitive       = main
